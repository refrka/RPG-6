class_name OldInventory extends Resource
...
## DEPRECATED
@export var equipment: Array[OldItemData]

@export var equipped_weapon: OldItemData

@export var equipped_armor: OldItemData

@export var equipped_accessory: OldItemData
...
## DEPRECATED
func add_item(item_id: StringName, quantity: int, _item_data: OldItemData = null) -> void:
...
static func load_dictionary(save_dict: Dictionary) -> OldInventory:

	var inventory = OldInventory.new()
	...
	for dict in save_dict["equipment"]:
		...
		inventory.equipment.append(OldItemData.load_dictionary(dict))

	inventory.equipped_weapon = OldItemData.load_dictionary(save_dict["equipped_weapon"])

	inventory.equipped_armor = OldItemData.load_dictionary(save_dict["equipped_armor"])

	inventory.equipped_accessory = OldItemData.load_dictionary(save_dict["equipped_accessory"])
